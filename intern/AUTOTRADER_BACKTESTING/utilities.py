...
class TradeAnalysis:
    ...
    def summary(self) -> dict:
        ...
        if len(self.account_history) > 0:
            ...
            day_max_draw = self.account_history.drawdown[self.account_history.drawdown == max_drawdown].count()

            long_pnl = self.account_history.long_pnl[-1]
            short_pnl = self.account_history.short_pnl[-1]
            self.account_history.commulative_pnl = self.account_history.long_pnl + self.account_history.short_pnl
            # print("commulative_pnl:-",self.account_history.commulative_pnl) 
            # print()
            print("self.account_history:-",self.analyse_account)
            print("self.order_history:-",self.order_history
            ,"self.position_summary:-",self.position_summary,
            "self.analyse_account:-",self.analyse_account )
            self.order_history.to_csv("order_hist.txt")
            self.trade_history.to_csv("trade_hist.txt")
            self.account_history.to_csv("account_hist.csv")
            self.position_history.to_csv("position_hist.csv")
            
            # self.account_history.commulative_pnl.to_csv("report.txt")
            ...
            trade_results["no_of_max_drawdown_day"] = day_max_draw
            trade_results["max_drawdown"] = max_drawdown
            trade_results["long_pnl"] = long_pnl
            trade_results["short_pnl"] = short_pnl

        # All trades
        no_trades = len(self.trade_history)
        avg_profit_trade = abs_return/no_trades
        trade_results["avg_profit_trade"] = avg_profit_trade
        ...
        if no_trades > 0:
            ...
            # print("sxdcfvgbhnj:-",self.isolated_position_history.profit)
            wins = self.position_history[
                
                self.position_history > 0
                
            ]
            print("self.position_history:-",self.position_history)
            # print("wins:-",wins)
            # wins.to_csv("wins.txt")
            avg_win = np.mean(wins)
            max_win = np.max(wins)

            print("avg_win:-",avg_win,"max_win:-",max_win)
            # Analyse losing positions
            loss = self.position_history[
                self.position_history < 0
            ]
            avg_loss = abs(np.mean(loss))
            max_loss = abs(np.min(loss))
            ...
            # print(" longest_win_streak:-",longest_win_streak," longest_lose_streak:-", longest_lose_streak)
            # try:
