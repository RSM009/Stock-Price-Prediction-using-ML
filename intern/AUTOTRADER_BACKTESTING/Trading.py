...
from autobot import AutoTraderBot
import datetime as datetime
   ...
   ##########################################  Backtesting Assignment ################################## 
    ...
    def Backtesting(
        ...
        if start_dt is None and end_dt is None:
            ...
            end_dt = f"{year:04d}-{month:02d}-{day:02d} 00:00:00+00:00"
            start_dt = parser.parse(start_dt)
            end_dt = parser.parse(end_dt)
            print("strt_dt:-",start_dt,"end_dt:-",type(end_dt)) 
   
            # start_dt = datetime.strptime(start + "+0530", "%d/%m/%Y %I:%M %p%z").astimezone(ist)
            # ??end_dt = datetime.strptime(end + "+0530", "%d/%m/%Y %I:%M %p%z").astimezone(ist)
...
################################### Configure the variables ######################################### 
    ...
    def configure(
        ...
        """Configures run settings for Trader.

        Parameters
        ----------
        verbosity : int, optional
            The verbosity of Trader (0, 1, 2). The default is 1.
        broker : str, optional
        ...
        instance_str : str, optional
            The name of the active Trader instance, used to control bots
            deployed when livetrading in continuous mode. When not specified,
            the instance string will be of the form 'Trader_instance_n'.
            The default is None.
        ...
        deploy_time : datetime, optional
            The time to deploy the bots. If this is a future time, Trader
...
##################################################### Strategy #######################################################
    ...
    def add_strategy(
        ...
        strategy : Trader Strategy, optional
        ...
        shutdown_method : str, optional
            ...
            method will be called when Trader is livetrading in continuous
        ...
        -------
        None
            The strategy will be added to the active Trader instance.
        """
    ...
    def print_trade_results(self, trade_results: TradeAnalysis = None) -> None:
        ...
        print("traderesult :-",trade_results)
        if "start" in trade_summary:
            
            print("trade_summary:-",trade_summary)
            ...
            long_pnl = trade_summary["long_pnl"]
            day_max_draw = trade_summary["no_of_max_drawdown_day"]
            short_pnl = trade_summary["short_pnl"]
            ...
            avg_profit_trade = trade_summary["avg_profit_trade"]
            if no_trades > 0:
            ...
            print("day_max_draw:            {}".format(day_max_draw))
            if no_trades > 0:
                ...
                print("Profit Long trades:         {}".format(round(long_pnl,2)))
                print("Profit Short trades:         {}".format(round(short_pnl,2)))
                print("avg_profit_trade:         ${}".format(round(avg_profit_trade, 3)))
    ...
    def _trade_update_loop(self):
        ...
        if len(self._bots_deployed) == 0:
            ...
            # Automated trading
            print("self._run_mode.lower()",self._run_mode.lower())
            if self._run_mode.lower() == "continuous":
                ...
                if self._backtest_mode:
                    ...
                    timestamp = self._data_start + self._warmup_period 
                    # datetime
                    # pbar = tqdm(
