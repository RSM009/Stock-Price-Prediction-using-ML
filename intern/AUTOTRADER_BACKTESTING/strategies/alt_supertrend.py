...
class SuperTrendScan:
    ...
    def generate_features(self, data: pd.DataFrame):
        ...
        signals[(data["Close"] > ema) & (st_df["trend"] == 1) & (st_df["trend"].shift() == -1)] = 1
        signals[(data["Close"] < ema) & (st_df["trend"] == -1) & (st_df["trend"].shift() == 1)] = -1
    ...
    def generate_signal(self, data):
        ...
        if rolling_signal[-1] == 1 and \
            candles_since_signal[-1] < self.parameters['candle_tol']:
            # Start of uptrend
            signal = 1
        
        elif rolling_signal[-1] == -1 and \
            candles_since_signal[-1] < self.parameters['candle_tol']:
            # Start of downtrend
            signal = -1
